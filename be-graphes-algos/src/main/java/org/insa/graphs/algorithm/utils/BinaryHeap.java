...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    private int indexOf(E x) throws ElementNotFoundException
    {
    	int foundIndex = this.array.indexOf(x);
    	
    	if (foundIndex == -1 || foundIndex >= this.currentSize)
    ...
    public void remove(E x) throws ElementNotFoundException {
    	...
    	int foundIndex = this.indexOf(x);
    	
		E lastElement = this.array.get(this.size() - 1);
		this.arraySet(foundIndex, lastElement);
		--this.currentSize;
		
		// percolateUp if smaller than its parent
		if (lastElement.compareTo(this.array.get(this.indexParent(foundIndex))) < 0)
			this.percolateUp(foundIndex);
		else
			this.percolateDown(foundIndex);
    }
